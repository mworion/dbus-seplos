...
class Alarm:
    ...
    cell_warning_byte = 3
    cell_temperature_warning_byte = 20
    ambient_temperature_warning_byte = 24
    component_temperature_warning_byte = 25
    dis_charging_current_warning_byte = 26
    pack_voltage_warning_byte = 27

    # info 20 bit alarm offsets
    warning_1_alarm_byte = 29
    warning_2_alarm_byte = 30
    warning_3_alarm_byte = 31
    warning_4_alarm_byte = 32
    warning_5_alarm_byte = 33
    warning_6_alarm_byte = 34
    power_status_byte = 35
    equalization_status1_byte = 36
    equalization_status2_byte = 37
    system_status_byte = 38
    disconnection_status1_byte = 39
    disconnection_status2_byte = 40
    warning_7_alarm_byte = 41
    warning_8_alarm_byte = 42
    ...
    def decode_data(self, data: bytes) -> None:
        ...
        for i in range(self.number_of_cells):
            self.cell_voltage_warning[i] = self.stat_24byte_alarm(
                data=data, offset=self.cell_warning_byte + i)

        for i in range(4):
            self.cell_temperature_warning[i] = self.stat_24byte_alarm(
                data=data, offset=self.cell_temperature_warning_byte + i)

        self.ambient_temperature_warning = self.stat_24byte_alarm(
            data=data, offset=self.ambient_temperature_warning_byte)
        self.component_temperature_warning = self.stat_24byte_alarm(
            data=data, offset=self.component_temperature_warning_byte)
        self.dis_charging_current_warning = self.stat_24byte_alarm(
            data=data, offset=self.dis_charging_current_warning_byte)
        self.pack_voltage_warning = self.stat_24byte_alarm(
            data=data, offset=self.pack_voltage_warning_byte)

        # warning 1
        self.voltage_sensing_failure = self.stat_20bit_alarm(
            data=data, offset=self.warning_1_alarm_byte, warn_bit=0)
        self.temp_sensing_failure = self.stat_20bit_alarm(
            data=data, offset=self.warning_1_alarm_byte, warn_bit=1)
        self.current_sensing_failure = self.stat_20bit_alarm(
            data=data, offset=self.warning_1_alarm_byte, warn_bit=2)
        self.power_switch_failure = self.stat_20bit_alarm(
            data=data, offset=self.warning_1_alarm_byte, warn_bit=3)
        self.cell_voltage_difference_sensing_failure = self.stat_20bit_alarm(
            data=data, offset=self.warning_1_alarm_byte, warn_bit=4)
        self.charging_switch_failure = self.stat_20bit_alarm(
            data=data, offset=self.warning_1_alarm_byte, warn_bit=5)
        self.discharging_switch_failure = self.stat_20bit_alarm(
            data=data, offset=self.warning_1_alarm_byte, warn_bit=6)
        self.current_limit_switch_failure = self.stat_20bit_alarm(
            data=data, offset=self.warning_1_alarm_byte, warn_bit=7)

        # warning 2
        self.cell_overvoltage = self.stat_20bit_alarm(
            data=data, offset=self.warning_2_alarm_byte, warn_bit=0,
            protection_bit=1)
        self.cell_voltage_low = self.stat_20bit_alarm(
            data=data, offset=self.warning_2_alarm_byte, warn_bit=2,
            protection_bit=3)
        self.pack_overvoltage = self.stat_20bit_alarm(
            data=data, offset=self.warning_2_alarm_byte, warn_bit=4,
            protection_bit=5)
        self.pack_voltage_low = self.stat_20bit_alarm(
            data=data, offset=self.warning_2_alarm_byte, warn_bit=6,
            protection_bit=7)

        # warning 3
        self.charging_temp_high = self.stat_20bit_alarm(
            data=data, offset=self.warning_3_alarm_byte, warn_bit=0,
            protection_bit=1)
        self.charging_temp_low = self.stat_20bit_alarm(
            data=data, offset=self.warning_3_alarm_byte, warn_bit=2,
            protection_bit=3)
        self.discharging_temp_high = self.stat_20bit_alarm(
            data=data, offset=self.warning_3_alarm_byte, warn_bit=4,
            protection_bit=5)
        self.discharging_temp_low = self.stat_20bit_alarm(
            data=data, offset=self.warning_3_alarm_byte, warn_bit=6,
            protection_bit=7)

        # warning 4
        self.ambient_temp_high = self.stat_20bit_alarm(
            data=data, offset=self.warning_4_alarm_byte, warn_bit=0,
            protection_bit=1)
        self.ambient_temp_low = self.stat_20bit_alarm(
            data=data, offset=self.warning_4_alarm_byte, warn_bit=2,
            protection_bit=3)
        self.component_temp_high = self.stat_20bit_alarm(
            data=data, offset=self.warning_4_alarm_byte, warn_bit=4,
            protection_bit=5)

        # warning 5
        self.charging_overcurrent = self.stat_20bit_alarm(
            data=data, offset=self.warning_5_alarm_byte, warn_bit=0,
            protection_bit=1)
        self.discharging_overcurrent = self.stat_20bit_alarm(
            data=data, offset=self.warning_5_alarm_byte, warn_bit=2,
            protection_bit=3)
        self.transient_overcurrent = self.stat_20bit_alarm(
            data=data, offset=self.warning_5_alarm_byte, warn_bit=4)
        self.output_short_circuit = self.stat_20bit_alarm(
            data=data, offset=self.warning_5_alarm_byte, warn_bit=5)
        self.transient_overcurrent_lock = self.stat_20bit_alarm(
            data=data, offset=self.warning_5_alarm_byte, warn_bit=6)
        self.output_short_circuit_lock = self.stat_20bit_alarm(
            data=data, offset=self.warning_5_alarm_byte, warn_bit=7)

        # warning 6
        self.charging_high_voltage = self.stat_20bit_alarm(
            data=data, offset=self.warning_6_alarm_byte, warn_bit=0)
        self.intermittent_power_supplement = self.stat_20bit_alarm(
            data=data, offset=self.warning_6_alarm_byte, warn_bit=1)
        self.soc_low = self.stat_20bit_alarm(
            data=data, offset=self.warning_6_alarm_byte, warn_bit=2,
            protection_bit=3)
        self.cell_low_voltage_forbidden_charging = self.stat_20bit_alarm(
            data=data, offset=self.warning_6_alarm_byte, warn_bit=4)
        self.output_reverse_protection = self.stat_20bit_alarm(
            data=data, offset=self.warning_6_alarm_byte, warn_bit=5)
        self.output_connection_failure = self.stat_20bit_alarm(
            data=data, offset=self.warning_6_alarm_byte, warn_bit=6)

        # power status
        self.discharge_switch = self.stat_20bit_alarm(
            data=data, offset=self.power_status_byte, on_off_bit=0)
        self.charge_switch = self.stat_20bit_alarm(
            data=data, offset=self.power_status_byte, on_off_bit=1)
        self.current_limit_switch = self.stat_20bit_alarm(
            data=data, offset=self.power_status_byte, on_off_bit=2)
        self.heating_limit_switch = self.stat_20bit_alarm(
            data=data, offset=self.power_status_byte, on_off_bit=3)

        # equalization status 1 + 2
        for i in range(self.number_of_cells):
            on_off_bit = i % 8
            offset = self.equalization_status1_byte if i < 8 else self.equalization_status2_byte
        ...
        self.discharge = self.stat_20bit_alarm(
            data=data, offset=self.system_status_byte, on_off_bit=0)
        self.charge = self.stat_20bit_alarm(
            data=data, offset=self.system_status_byte, on_off_bit=1)
        self.floating_charge = self.stat_20bit_alarm(
            data=data, offset=self.system_status_byte, on_off_bit=2)
        self.standby = self.stat_20bit_alarm(
            data=data, offset=self.system_status_byte, on_off_bit=4)
        self.power_off = self.stat_20bit_alarm(
            data=data, offset=self.system_status_byte, on_off_bit=5)
        # disconnection status 1 + 2
        for i in range(self.number_of_cells):
            warn_bit = i % 8
            offset = self.disconnection_status1_byte if i < 8 else self.disconnection_status2_byte
        ...
        self.auto_charging_wait = self.stat_20bit_alarm(
            data=data, offset=self.warning_7_alarm_byte, warn_bit=4)
        self.manual_charging_wait = self.stat_20bit_alarm(
            data=data, offset=self.warning_7_alarm_byte, warn_bit=5)

        # warning 8
        self.eep_storage_failure = self.stat_20bit_alarm(
            data=data, offset=self.warning_8_alarm_byte, warn_bit=0)
        self.rtc_clock_failure = self.stat_20bit_alarm(
            data=data, offset=self.warning_8_alarm_byte, warn_bit=1)
        self.no_calibration_of_voltage = self.stat_20bit_alarm(
            data=data, offset=self.warning_8_alarm_byte, warn_bit=2)
        self.no_calibration_of_current = self.stat_20bit_alarm(
            data=data, offset=self.warning_8_alarm_byte, warn_bit=3)
        self.no_calibration_of_null_point = self.stat_20bit_alarm(
            data=data, offset=self.warning_8_alarm_byte, warn_bit=4)
